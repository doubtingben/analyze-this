...
if [ -z "$1" ]; then
  ...
  echo ""
  echo "Note: analysis, normalize, follow_up, and podcast_audio workers are now deployed as Cloud Run Jobs."
...
if [ "$JOB_TYPE" != "manager" ]; then
  ...
  echo "For worker jobs (analysis, normalize, follow_up, podcast_audio), use:"
  echo "  ./backend/scripts/deploy-worker-job.sh <analysis|normalize|follow_up|podcast_audio|all>"
  exit 1
fi
