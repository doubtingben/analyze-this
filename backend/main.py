...
@app.get("/login", dependencies=[Depends(RateLimiter(requests=10, window=60))])
...
@app.post("/api/share", dependencies=[Depends(check_csrf), Depends(RateLimiter(requests=20, window=60))])
...
@app.post("/api/items/{item_id}/notes", dependencies=[Depends(check_csrf), Depends(RateLimiter(requests=20, window=60))])
...
@app.get("/api/search", dependencies=[Depends(RateLimiter(requests=20, window=60))])
